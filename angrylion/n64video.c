...
#define byteswap_32(x) ((uint32_t) (((x) >> 24 & 0xff) | ((x) >> 8 & 0xff00) | ((x) << 8 & 0xff0000) | ((x) << 24 & 0xff000000)))
...
#define rsp_imem ((uint32_t*)rsp.imem.data)
#define rsp_dmem ((uint32_t*)rsp.dmem.data)

#define rdram ((uint32_t*)(rdram.ram.data))
#define rdram16 ((uint16_t*)(rdram.ram.data))
#define rdram8 ((uint8_t*)(rdram.ram.data))

#define vi_width (vi.io.width)

#define dp_start (rdp.command.start)
#define dp_end (rdp.command.end)
#define dp_current (rdp.command.current)
#define dp_status (rdp.status)
...
static SPAN span[1024] __attribute__((aligned(16)));
uint8_t cvgbuf[1024];
...
//print(hex(rdp_cmd_data[rdp_cmd_cur+0], 8L), hex(rdp_cmd_data[rdp_cmd_cur+1], 8L), " @ ", hex(dp_current, 8L), "\n");
